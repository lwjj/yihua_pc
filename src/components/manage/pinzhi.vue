          ...
          this.$getMessage(res1 => {
            if (this.userInfo.organType == 3) {
              for (let val of res.data.propertyManagement.subclass) {
                if (val.alias === "qualityManage") {
                  for (var v of val.subclass) {
                    switch (v.alias) {
                      case "emergency":
                        v.num = res1.data.proManageMr.emergencyNum * 1;
                        break;
                      case "examine":
                        v.num = res1.data.proManageMr.examineNum * 1;
                        break;
                      case "meeting":
                        v.num = res1.data.proManageMr.meetingNum * 1;
                        break;
                      case "notice":
                        v.num = res1.data.proManageMr.noticeNum * 1;
                        break;
                      default:
                        console.info("品质管理");
                    }
