<template>
  <yd-layout class="layout_my">
      <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX" v-title="'投诉建议'" title="投诉建议" color="#fff" :bgcolor="mainColor" height=".88rem">
      <span @click="back" class="addClickArea" slot="left">
          <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
        </span>
        <div slot="right" class="addRightClickArea edit" @click="$router.push('/estate/serviceList/complain/add_complain')"  style="color: #fff;font-size:.6rem;">+</div>
      </yd-navbar>
     <div class="headTab" slot="top">
      <div class="flex-row h9">
        <div :class='["flex1","jusCenter" ,{active:currentIndex == 0}]' :style="currentIndex == 0?`color:${mainColor};border-bottom:2px solid ${mainColor}`:``" @click="chooseIndex(0)">
          <span>起止日期</span>
           <yd-icon name='toDown' @click="chooseIndex(0)" v-if="currentIndex != 0" size='.2rem' color='#444' custom></yd-icon>
           <yd-icon name='shangla' @click="chooseIndex(0)" v-if="currentIndex == 0" :color='mainColor' size='.2rem' custom></yd-icon>
        </div>
        ...
        <div :class='["flex1","jusCenter" ,{active:currentIndex == 2}]' :style="currentIndex == 2?`color:${mainColor};border-bottom:2px solid ${mainColor}`:``" @click="chooseIndex(2)">
          <span>类型</span>
      ...
      <div class="filter" @click.stop="''">
        ...
        <div class="timeFilter" v-show="currentIndex == 0">
         ...
         <div class="flex-row statusFilte flex-start" v-show="currentIndex == 2">
           <yd-button v-for="(item,index) in typeArr" :key="index" @click.native="currentIndex2 == index?currentIndex2= -1:currentIndex2 = index" type='hollow' :style="currentIndex2 == index?`border:1px solid ${mainColor};color:${mainColor}`:``" :class="['myButton',{active:index == currentIndex2}]" >
             {{item.name}}
           </yd-button>
         </div>
          ...
          <div class="list bgwhite pdlr2 mt2" v-for="(item,index) in contentList" :key="index">
            <ul class="font26 flex-row" style="color:#333;border-bottom:1px dashed #ddd;justify-content:space-between">
              ...
              <li>
                ...
                <span class="state state2" v-if="item.status == 1">上报中</span>
                <span class="state state2" v-if="item.status == 2">已接单</span>
                <span class="state state2" v-if="item.status == 3">处理中</span>
                <span class="state state2" v-if="item.status == 4">待确认</span>
                <span class="state state1" v-if="item.status == 5">完成</span>
                <span class="state stateClose" v-if="item.status == 6">关闭</span>
                <span class="state state0" v-if="item.status == 7">不同意</span>
                <!-- <span class="pdlr1 state state6" v-if="item.status == 6">客服关闭 </span> -->
              </li>
            </ul>
            <div class="flex-row-nowrap" style="padding:.2rem 0;" @click="turnTo('/estate/serviceList/wuye/complain_detail?id=' + item.guid)">
              <img  class="document" src="../../../../../assets/images/rent/estate/document.png" alt="">
              <div style="flex:1;">
                <!-- {{detailContent}} -->
                <div class="font30"><span class="nowrap">{{item.detailContent.length > 20?item.detailContent.substr(0,20) + '...':item.detailContent}}</span></div>
                <div class="flex font26 foCo9 mt1">
                  <span>{{item.customerName}}</span>
                  <span class="flex"><yd-icon name='rili' custom size='.4rem'></yd-icon>{{item.createDate.substr(0,10)}}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    getMyInvitation() {
      ...
      if (this.currentIndex2 > 0) {
        data.type = this.typeArr[this.currentIndex2].value;
      } else {
        data.type = "";
      }
      this.xhr
        .post(
          // `${this.subUrl.activity}/complaint/queryComplaint`,
          `${this.subUrl.activity}/complaint/listComplaint`,
