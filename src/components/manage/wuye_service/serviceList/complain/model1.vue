<template>
  <yd-layout class="layout_my">
    <div class="blue_box" v-show="!userAgent.isWX">
      <yd-navbar class="header" v-title="'投诉建议'" title="投诉建议" color="#fff" :bgcolor="mainColor" height=".88rem">
         <span slot="left" class="addClickArea" @click="back">
            <span><yd-navbar-back-icon color="#fff"></yd-navbar-back-icon></span>
        </span>
        <div slot="right"  @click="turnTo('/estate/serviceList/complain/add_complain')" class="edit" style="color: #fff;font-size:.6rem;">+</div>
      </yd-navbar>
    </div>
    <div class="search">
        <div>
    ...
    <div class="contentList">
        <div class="content" v-for="(item,index) in contentList" :key="index">
              <div class="top">
                    <p><i><img src="../../../../../assets/images/rent/estate/document.png" alt=""></i>{{item.list}}</p>
                    <span>单号：{{item.guid}}</span>
              </div>
              <div class="bottom">
                   <i><img src="../../../../../assets/images/rent/estate/document.png" alt=""></i>
                   <p @click="turnTo('/estate/serviceList/complain/complain_detail')"><span>{{item.msg}}</span><a>关键字：{{item.ress}}</a></p>
                   <span>
