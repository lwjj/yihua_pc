  ...
  <div style="height:100vh">
    <yd-layout class="layout_my" v-if="showUser && !showReplay">
      ...
      <div class="bgwhite h9 pdlr2 text-left flex center" v-if="result.complaint"
        ...
        <span class="state state0" v-if="result.complaint.status == 2">已接单</span>
        ...
        <span class="state state2" v-if="result.complaint.status == 5">待评价</span>
        <span class="state state1" v-if="result.complaint.status == 6">完成</span>
        <span class="state stateClose" v-if="result.complaint.status == 7">关闭</span>
      </div>
      ...
      <div class="div-row" v-if="result.complaint">
        <div>单号</div>
        {{result.complaint.guid}}
        <a class="pull-right hide-or-show" :style="{color:mainColor}" v-if="hide" @click="hideDiv" >隐藏</a>
        <a class="pull-right hide-or-show" :style="{color:mainColor}"  v-if="!hide" @click="hideDiv">展开</a>
      </div>
      ...
      <yd-timeline style="margin-top:.2rem;">
        <yd-timeline-item v-for='(item,index) in result.tasksRecordList' v-if="result.tasksRecordList && result.tasksRecordList.length > 0" :key='index'>
          ...
          <div class="font28" v-if='item.remark'>
            <span>备注:</span>
