<template>
   <div class="" style="padding:20px;background:#fefefe;height:100%;box-sizing:border-box;overflow:auto;">
      <div class="title">
     ...
     <div class="content">
       <div class="tips bgwhite"><b>行业类型</b></div>
         ...
         <div class="list">
           ...
           <div class="subContent" v-show="item.show">
             <yd-button class="buttonChoose" v-for="(item1,key1) in item.subclass" :key="key1"
                        :bgcolor='item1.show?mainColor:"#fff"'  @click.native="choose_free(key,key1)">{{item1.name}}{{mainColor}}{{item1.show}}
             </yd-button>
           </div>
         </div>
       </div>
      ...
      this.xhr
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            this.options_free = [];
            this.options_require = res.data.data.pmc
              .concat(res.data.data.icc)
              .concat(res.data.data.pm);
          }
        });
    },
    cancel() {
