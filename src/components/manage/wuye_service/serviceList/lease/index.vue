<template>
  <yd-layout class="layout_my">
    <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX"  title="租赁" color="#fff" :bgcolor="mainColor" height=".88rem">
      <span @click="back" class="addClickArea" slot="left">
          <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
        </span>
      <div slot="right" @click="$router.push('/estate/serviceList/lease/add')" class="edit addRightClickArea" style="color: #fff;font-size:.6rem;">+</div>
      <!--<span @click="turnTo('/estate/serviceList/exit_rent/declares')" slot="right" class="edit addRightClickArea" style="color: #fff;font-size:.6rem;">+</span>-->
    </yd-navbar>
    <div class="headTab" slot="top">
      <div class="flex-row h9">
        <div :class='["flex1","jusCenter" ,{active:currentIndex == 0}]'
             :style="currentIndex == 0?`color:${mainColor};border-bottom:2px solid ${mainColor}`:``"
             @click="chooseIndex(0)">
          <span>起止日期</span>
        ...
        <div :class='["flex1","jusCenter" ,{active:currentIndex == 1}]'
             :style="currentIndex == 1?`color:${mainColor};border-bottom:2px solid ${mainColor}`:``"
             @click="chooseIndex(1)">
          <span>全部状态</span>
      ...
      <div class="contentList" slot="list">
        <div class="list bgwhite pdlr2 mt2" v-for="(item,index) in contentList" :key="index">
          <!--<ul class="font26 flex-row" style="color:#333;border-bottom:1px dashed #ddd;justify-content:space-between">-->
            ...
            <!--</li>-->
            <!--<li>-->
              <!--<span v-if="item.status == 0" class="state stateClose">关闭</span>-->
              <!--<span v-if="item.status == 1" class="state state1">完成</span>-->
              <!--<span v-if="item.status == 2" class="state state2">客服处理</span>-->
              <!--<span v-if="item.status == 3" class="state state2">经理指派</span>-->
              <!--<span v-if="item.status == 4" class="state state2">待抄表</span>-->
              <!--<span v-if="item.status == 5" class="state state2">已抄表</span>-->
              <!--&lt;!&ndash;<span class="pdlr1 state state0" v-if="item.status == 0">关闭</span>&ndash;&gt;-->
              <!--&lt;!&ndash;<span class="pdlr1 state state1" v-if="item.status == 1">完成</span>&ndash;&gt;-->
              <!--&lt;!&ndash;<span class="pdlr1 state state2" v-if="item.status == 2">经理审批</span>&ndash;&gt;-->
              <!--&lt;!&ndash;<span class="pdlr1 state state3" v-if="item.status == 3">客服发布</span>&ndash;&gt;-->
              <!--&lt;!&ndash;<span class="pdlr1 state state4" v-if="item.status == 4">租户处理</span>&ndash;&gt;-->
              <!--&lt;!&ndash;<span class="pdlr1 state state5" v-if="item.status == 5">复查</span>&ndash;&gt;-->
              <!--&lt;!&ndash;<span class="pdlr1 state state6" v-if="item.status == 6">客服关闭 </span>&ndash;&gt;-->
            <!--</li>-->
          <!--</ul>-->
          <div class="flex-row-nowrap" style="padding:.2rem 0;" @click="turnTo('/estate/serviceList/wuye/lease/details?id=' + item.guid)">
            <img  class="document" src="../../../../../assets/images/rent/estate/document.png" alt="">
            <div style="flex:1;">
              <div class="font30"><span class="nowrap">{{item.buildingName.length > 20?item.buildingName.substr(0,20) + '...':item.buildingName}}</span></div>
              <div class="flex font26 foCo9 mt1">
                <span>区域：{{item.district.districtName}}-{{item.district.areaName}}</span>
                <span class="flex"><yd-icon name='rili' custom size='.4rem'></yd-icon>{{item.createTime.substr(0,10)}}</span>
              </div>
            </div>
          </div>
          <div class="flex-row-nowarp h7 bt" v-if="item.contactsInfo" >
            <div class="flex foCo9">
              <img class="mr1" src="../../../../../assets/images/rent/ent/human.png" width="10" alt="">
              <span class="mr2">{{item.contactsInfo.name.length > 3?item.contactsInfo.name.substr(0,3) + '...':item.contactsInfo.name}}</span>
              <span>{{item.contactsInfo.account}}</span>
            </div>
            <div class="state state1" style="padding:.06rem .1rem;" :style="`border-color:${mainColor}`" @click="contact(item.contactsInfo.account)">
              <yd-icon name="tel2" size=".3rem" :color="mainColor" custom></yd-icon>
              <span :style="{color:mainColor}">联系</span>
            </div>
          </div>
        </div>
      </div>
    ...
    methods: {
      ...
      getMyInvitation() {
        ...
        if(this.currentIndex1 > -1){
          if(this.statusArr[this.currentIndex1].value > -1){
            data.status = this.statusArr[this.currentIndex1].value;
          }
        }
        this.xhr
          .post(
            `${this.subUrl.activity}/lease/getLeaseList`,
            qs.stringify(data)
          )
          .then(res => {
            if (res.data.code == 1) {
