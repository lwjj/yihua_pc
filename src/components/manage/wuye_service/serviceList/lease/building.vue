  ...
  <yd-layout class="yd-layout" style="width:750px;">
    ...
    <div class="title">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item  @click.native="close">返回</el-breadcrumb-item>
        <el-breadcrumb-item  @click.native="close">选择大厦</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <!-- <div class="relative">
      <div class="headTab" slot="top">
      <div class="flex-row" style="height:.9rem">
    ...
    <div class="flex bd" style="width:100%;padding:10px 20px;" >
      <el-select v-model="value"   @change="change" class="flex1 mr1" clearable placeholder="地区">
          <el-option
    ...
    <div class="mask" style="top:1.88rem" v-if="isFilter" @click="cancelIndex()">
    ...
    <div>
      <el-button type="primary" @click="confirm" class="mt2 mb2 ml2">
        确定
      </el-button>
    </div>
    <div class="flex-row-nowarp h9 font32 bt jusCenter bgwhite" slot="bottom" :style="{color:mainColor}"></div>
    </div>
  </yd-layout>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleItemChange(val) {
      if (val.length == 1) {
        this.getDistrict(this.options1[val[0]].guid, res => {
          this.options1[val[0]].children = res;
        });
      } else if (val.length == 2) {
        this.getDistrict(this.options1[val[0]].children[val[1]].guid, res => {
          this.options1[val[0]].children[val[1]].children = res;
        });
      } else if (val.length == 3) {
        this.getDistrict(
          this.options1[val[0]].children[val[1]].children[val[2]].guid,
          res => {
            this.options1[val[0]].children[val[1]].children[
              val[2]
            ].children = res;
          },
          true
    ...
    getList() {
      ...
      if (this.value) {
        data.positionCondition = this.value;
      }
      if (this.value1.length > 1) {
        data.scopeValue = this.options1[this.value1[0]].children[
          this.value1[1]
        ].children[this.value1[2]].guid;
      }
