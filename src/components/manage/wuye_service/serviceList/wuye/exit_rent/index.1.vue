<template>
  <yd-layout class="layout_my">
    <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX" v-title="'退租'" title="退租" color="#fff" :bgcolor="mainColor" height=".88rem">
      <span @click="back" slot="left">
          <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
        </span>
      <!--<div slot="right" v-if="userInfo.organType == 3" @click="$router.push('/estate/serviceList/wuye/rectification/add')" class="edit" style="color: #fff;font-size:.6rem;">+</div>-->
      <span @click="turnTo('/estate/serviceList/wuye/exit_rent/declares')" slot="right" class="edit addRightClickArea" style="color: #fff;font-size:.6rem;">+</span>
    </yd-navbar>
    ...
    <yd-infinitescroll :callback="loadList" ref="infinitescrollDemo">
      <div class="contentList" slot="list">
        <div class="list bgwhite pdlr2 mt2" v-for="(item,index) in contentList" :key="index">
          ...
          <div class="flex-row-nowrap" style="padding:.2rem 0;" @click="turnTo('/estate/serviceList/exit_rent/details?id=' + item.guid)">
            <img  class="document" src="../../../../../../assets/images/rent/estate/document.png" alt="">
            <div style="flex:1;">
              <div class="font30"><span class="nowrap">{{item.clientInfo.organName.length > 20?item.clientInfo.organName.substr(0,20) + '...':item.clientInfo.organName}}</span></div>
              <div class="flex font26 foCo9 mt1">
                <span>{{item.clientInfo.ridName}}{{item.clientInfo.unitName}}</span>
