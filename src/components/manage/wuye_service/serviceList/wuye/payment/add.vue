...
<div class="newRightContent" style="overflow:auto">
  <div class="bgwhite border">
    ...
    <div v-show='father'>
      ...
      <el-radio-group style="padding:20px 10px 0" v-model="submitType">
        <el-radio :label="2">图片上传</el-radio>
        <el-radio :label="1">手动输入</el-radio>
      </el-radio-group>
      <!-- 项目单 -->
      <div class="tableBox" v-show="submitType == 1">
        <div class="table" v-if="submitData.payProject.length > 0" :style="$route.params.id == 1?`width:12.6rem;`:`width:7.5rem`">
          <!-- 标题 -->
      ...
      <!-- 图片附件 -->
          <div class="upload_photo" v-show="submitType == 2">
            <el-upload
            :action="uploadUrl"
      ...
      <!-- 备注 -->
      <div class="remarks">
        <div class="flex-row" style='padding:10px;'>备注</div>
        <el-input slot="right" type="textarea" :rows='5' resize="none" class="pd3 bgwhite" style="line-height:.5rem;padding:.2rem 0.2rem .5rem;" placeholder="请输入备注" v-model='submitData.explain'
    ...
    <div class="bgwhite jiaofei" v-show='!father && $route.params.id == 1'>
      ...
      <div class="flex-row flex-start h7" size='large'>
        ...
        <div class="label"></div>
        <el-button type="hollow" @click.native='cancel'>取消</el-button>
        <el-button type="primary" @click.native='savePay'>保存</el-button>
      </div>
    ...
    <div class="bgwhite jiaofei" v-show='!father && $route.params.id == 2'>
      ...
      <el-button type='primary' class="button mt2 ml2 mb2" @click.native='submit' v-if="father">提交</el-button>
  </div>
  </div>
</div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      var uploadArr = [];
      for (let key in this.uploadHash) {
        uploadArr.push(this.uploadHash[key]);
      }
      ...
      if (this.submitType == 1) {
        ...
        if (uploadArr.length == 0) {
          return this.$message.error("请上传图片附件");
        }
      }
      data.accessorys = uploadArr;
