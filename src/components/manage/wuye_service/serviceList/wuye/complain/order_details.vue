...
<div class="newRightContent" style="overflow:auto">
  <div class="bgwhite border">
    <yd-layout class="layout_my yd-layout"  style='width:750px;' v-if="showUser && !showReplay">
      <div class="title">
        <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{path: '/manage/wuye_service'}">物业服务单</el-breadcrumb-item>
            <el-breadcrumb-item  @click.native='$back'>投诉建议</el-breadcrumb-item>
            <el-breadcrumb-item>投诉建议详情</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      ...
      <!-- <yd-navbar class="header"  slot="navbar" v-show="!userAgent.isWX" v-title="'投诉建议详情'" title="投诉建议详情" color="#fff" :bgcolor="mainColor" height=".88rem">
        <a @click="back" slot="left" class="addClickArea"> 
      ...
      <!--</div>-->
      <div class="bgwhite h9 pdlr2 text-left flex" v-if="result.complaint"
          style="border-bottom: 1px solid #e6e6e6;font-size: 16px;justify-content:space-between;padding:0 40px 0 50px">
      ...
      <div v-if="hide&&result.complaint">
        <div class="div-row">
          <div>报告人</div>
          {{result.complaint.applyUserName }}
        </div>
        <div class="div-row">
          <div>具体位置</div>
      ...
      <div class="order-detail">
        ...
        <br>
         ...
         <yd-lightbox  class="img-flex" :num='result.commAccessoryList.length' v-if="result.commAccessoryList">
            <div style="width: 20%;" v-for="(item,index) in result.commAccessoryList" :key="index">
              ...
              <yd-lightbox-img v-if="index < result.commAccessoryList.length<=5" :src="item.path"></yd-lightbox-img>
            </div>
        </yd-lightbox>
        <!-- <div class="img-flex">
      ...
      <div class="mask" v-if="showEvaluate" @click="showEvaluate=false">
        <div class="bgwhite" style="padding: 0.5rem .8rem .5rem" @click.stop="''">
          ...
          <el-card class="box-card">
            ...
            <el-input name="" type='textarea' :max='300' class='mt2' style="width:100%;" resize='none'  :rows='3' size='medium' v-model="comment"  placeholder="请输入评语"></el-input>
             <el-button type="primary" class="mt2" @click.native='pingjia'>发布评价</el-button>
        </div>
      </div>
      <!-- 完成评价 -->
      <div class="flex-row h8 center jusCenter" @click="showEvaluate = true" v-if="result.tasks && result.complaint.status == 5 && result.tasks.name == '待评价'" :style="{background:mainColor,color:'#fff',margin:'.3rem .5rem','border-radius':'.1rem'}">
        评价
      </div>
        <!-- 客服处理 -->
