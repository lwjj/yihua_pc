<template>
  <yd-layout class="layout_my">
      <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX" v-title="'接待服务'" title="接待服务" color="#fff" :bgcolor="mainColor" height=".88rem">
      <span @click="back" class="addClickArea" slot="left">
          <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
        </span>
        <div slot="right" @click="$router.push('/estate/serviceList/wuye/reception/declares')" class="edit" style="color: #fff;font-size:.6rem;">+</div>
      </yd-navbar>
     <div class="headTab" slot="top">
      <div class="flex-row h9">
        ...
        <!-- <div :class='["flex1","jusCenter" ,{active:currentIndex == 2}]' @click="chooseIndex(2)">
          <span>类型</span>
    ...
    <yd-infinitescroll :callback="loadList" ref="infinitescrollDemo">
         <div class="contentList" slot="list">
          <div class="list bgwhite pdlr2 mt2" v-for="(item,index) in contentList" :key="index">
            ...
            <div class="flex-row-nowrap" style="padding:.2rem 0;" @click="turnTo('/estate/serviceList/wuye/reception/details/888#'+item.guid)">
              <img  class="document" src="../../../../../../assets/images/rent/estate/document.png" alt="">
              <div style="flex:1;">
                <div class="font30">
                  <!-- {{item.detailContent}} -->
                  <span class="nowrap">{{item.detailContent.length > 20?item.detailContent.substr(0,20) + '...':item.detailContent}}</span>
                </div>
