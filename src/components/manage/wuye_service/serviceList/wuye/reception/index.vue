   ...
   <div class="layout_my newRightContent">
    <div class="title">
       <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>接待</el-breadcrumb-item>
        </el-breadcrumb>
    </div> 
    <div class="tableBox" style="padding-top:10px;">
      <div> 
        ...
        <el-button-group v-if="showBuild" style="float:right;margin-right:10px;">
          <el-button icon='el-icon-plus'  @click="$router.push('/estate/serviceList/wuye/reception/declares')">新建</el-button> 
        </el-button-group>
      ...
      <el-table :data="orderList" height="250" border style="width: 100%">
        <el-table-column prop="guid" label="单号" min-width="22%" align="center"></el-table-column>
        <el-table-column prop="detailContent" label="内容" show-overflow-tooltip min-width='18%'> </el-table-column> 
        <el-table-column prop="createDate" label="创建时间" show-overflow-tooltip	 min-width="21%" align="center">  </el-table-column> 
        <el-table-column prop="receptionDate" label="接待时间" show-overflow-tooltip	 min-width="21%" align="center">  </el-table-column> 
        <el-table-column label="机构" align="center"  min-width='27%'>
          <template slot-scope="scope" v-if="scope.row">
            {{scope.row.customerName}}
          </template>
        </el-table-column>     
        <el-table-column label="状态" align="center"  min-width='9%'>
          <template slot-scope="scope"> 
            <span class="state state2" v-if="scope.row.status == 1">上报中</span>
            <span class="state state2" v-if="scope.row.status == 2">已接单</span>
            <span class="state state2" v-if="scope.row.status == 3">处理中</span>
            <span class="state state2" v-if="scope.row.status == 4">待确认</span>
            <span class="state state1" v-if="scope.row.status == 5">完成</span>
            <span class="state stateClose" v-if="scope.row.status == 6">关闭</span>
            <span class="state state0" v-if="scope.row.status == 7">不同意</span> 
          </template>
        </el-table-column> 
        <el-table-column label="详情" align="center"  min-width='9%'> 
          <template slot-scope="scope">
            <el-button  plain size="small" @click="turnTo('/estate/serviceList/wuye/reception/details?guid=' + scope.row.guid)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
      ...
      <div id="printcode" v-show="print1">
        <el-table :data="orderList" border :style="{height:orderList.length*49+48+'px'}">
          <el-table-column prop="guid" label="单号" width="202px" align="center"></el-table-column>
          <el-table-column prop="detailContent" align="center" label="内容" show-overflow-tooltip width='340px'> </el-table-column> 
          <el-table-column prop="createDate" label="创建时间" show-overflow-tooltip	 width='202px' align="center">  </el-table-column> 
          <el-table-column prop="receptionDate" label="接待时间" show-overflow-tooltip	 width='202px' align="center">  </el-table-column> 
          <el-table-column label="机构" align="center"  width='270px'>
            <template slot-scope="scope" v-if="scope.row">
              {{scope.row.customerName}}
            </template>
          </el-table-column>     
          <el-table-column label="状态" align="center"  width='148px'>
            <template slot-scope="scope"> 
              <span class="state state2" v-if="scope.row.status == 1">上报中</span>
              <span class="state state2" v-if="scope.row.status == 2">已接单</span>
              <span class="state state2" v-if="scope.row.status == 3">处理中</span>
