   ...
   <div class="layout_my newRightContent">
    <div class="title">
       <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>整改</el-breadcrumb-item>
        </el-breadcrumb>
    </div> 
    <div class="tableBox" style="padding-top:10px;">
      <div> 
        ...
        <el-button-group v-if="showBuild" style="float:right;margin-right:10px;">
          ...
          ' @click="$router.push('/estate/serviceList/wuye/rectification/add')" >新建</el-button> 
        </el-button-group>
      ...
      <el-table :data="orderList" height="250" border style="width: 100%">
        ...
        <el-table-column prop="content" label="内容" show-overflow-tooltip min-width="30%"> </el-table-column> 
        <el-table-column prop="createTime" label="创建时间" show-overflow-tooltip min-width="18%" align="center"> </el-table-column> 
        <el-table-column label="机构" min-width="13%" align="center" prop='customerInfo.buildingName' ></el-table-column> 
        <el-table-column label="状态" min-width="11%" align="center">
          <template slot-scope="scope"> 
            <span class="pdlr1 state state0" v-if="scope.row.status == 0">关闭</span>
            <span class="pdlr1 state state1" v-if="scope.row.status == 1">完成</span>
            <span class="pdlr1 state state2" v-if="scope.row.status == 2">经理审批</span>
            <span class="pdlr1 state state3" v-if="scope.row.status == 3">客服发布</span>
            <span class="pdlr1 state state4" v-if="scope.row.status == 4">租户处理</span>
            <span class="pdlr1 state state5" v-if="scope.row.status == 5">复查</span>
            <span class="pdlr1 state state6" v-if="scope.row.status == 6">客服关闭 </span>  
          </template>
        </el-table-column> 
        <el-table-column label="详情" min-width="10%" align="center"> 
          <template slot-scope="scope">
            <el-button  plain size="small" @click="turnTo('/estate/serviceList/wuye/rectification/details?id=' + scope.row.guid)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
      ...
      <div id="printcode" v-show="print1">
        <el-table :data="orderList" border :style="{height:orderList.length*49+48+'px'}">
          ...
          <el-table-column prop="content" align="center" label="内容" show-overflow-tooltip width="500px"> </el-table-column> 
          <el-table-column prop="createTime" label="创建时间" show-overflow-tooltip width="240px" align="center"> </el-table-column> 
          <el-table-column label="机构" width="223px" align="center" prop='customerInfo.buildingName' ></el-table-column> 
