...
<div class="newRightContent" style="overflow:auto">
  <div class="bgwhite border">
    <yd-layout class="yd-layout" style="width:750px;" v-if="!showReplay">
     ...
     <div class="title">
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item  @click.native='$back'>整改单</el-breadcrumb-item>
            <el-breadcrumb-item>整改单详情</el-breadcrumb-item>
        </el-breadcrumb>
     </div>
    ...
    <div class="feedback" v-if="info && info.feedback">
     ...
     <div class="feedback" v-if="info && info.rectificationBill.status == 3 && info.tasks">
       <span>整改结果</span>
       <div class="flex-row flex-start ml2 mr2" style="line-height:2" >
         <span>限你单位于</span><br>
         <!-- <yd-datetime type='datetime' class="border" style="dispaly:inline-block;width:3rem" v-model="endTime"></yd-datetime> -->
         <span>
         ...
         时前进行整改，如我司人员在限期后仍发现贵司未整改的，我司将采取相关措施，
        并根据你我双方之约定处罚违约金<el-input type="number" v-model="money" class="mr2" style="width:2rem;display:inline-block"> </el-input>元。</div>
    </div>
    <div class="feedback" v-if="info && info.rectificationBill.status == 4">
       <span class="font32">整改结果</span>
       <div class="flex-row flex-start ml2 mr2" style="line-height:2" >{{info.rectificationBill.replyConten}}</div>
    </div>
    ...
    <yd-timeline v-if="info" class="mt2">
        <yd-timeline-item  v-for='(item,index) in info.tasksRecordList' :key='index'>
          ...
          <div class="font28 flex flex-start">
            <span>操作人:</span>
    ...
    <!-- 租户确定 -->
    <div class='buttons'  v-if="info && info.rectificationBill.status == 4 && info.tasks"><button  @click="rentConfirm" :style="{background:mainColor}">已整改</button></div>
    ...
    <!-- 客服关闭 -->
    <div class="flex-row h9 bgwhite bt font32" v-if='(info && info.rectificationBill.status==6) && info.tasks' slot='bottom'>
      <div class="flex1 jusCenter br" @click="csClose">
        <img style="width:.4rem;height:.4rem;" class="mr1" src="../../../../../../assets/images/rent/estate/disagree.png" alt="">
        <span style='color:#ff483f;'>客服关闭</span>
      </div>
    </div>
    <!-- 复查！ -->
  ...
  methods: {
    ...
    agree() {
      ...
      var data = {
        ...
        reslut: `限你单位于${
          this.endTime
        }:09时前进行整改，如我司人员在限期后仍发现贵司未整改的，我司将采取相关措施，
    ...
    // 租户确定
    rentConfirm() {
      this.$confirm("是否确定整改完成", { type: "warn" }).then(() => {
        var data = {
          token: this.$getkey(),
          userId: this.userInfo.guid,
          rectificationId: this.info.rectificationBill.guid,
      ...
      this.xhr
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            this.$setImg(res.data.data.commAccessoryList, this);
