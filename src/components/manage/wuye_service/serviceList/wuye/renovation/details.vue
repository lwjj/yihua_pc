...
<div class="newRightContent" style="overflow:auto">
  <div class="bgwhite border" v-if="showUser" style="height:100%;overflow:auto">
    <yd-layout class="layout_my yd-layout" style="width:750px;padding:0 0 30px;" v-if="showUser && showInput">
      <div class="title">
         <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{path: '/manage/wuye_service'}">物业服务单</el-breadcrumb-item>
            <el-breadcrumb-item  @click.native='$back'>装修单</el-breadcrumb-item>
            <el-breadcrumb-item>装修单详情</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
    <div class="organInfos">
      <div class="name">
        <p class="font600 font16">{{decoration.ridName}}{{decoration.unitName}}</p>
        <!-- <span v-show="decoration.status!=null" class="font22" :style="{border:'1px solid '+mainColor,color:mainColor}">{{decoration.status}}</span> -->
        <span v-if="decoration.status == 1" class="pdlr1 state state2">申请中</span>
        <span v-if="decoration.status == 2" class="pdlr1 state state3">待审批</span>
        <span v-if="decoration.status == 3" class="pdlr1 state state2">装修中</span>
      ...
      <div class="info">
         <ul>
          ...
          <li v-if="decoration.endDate"><span>装修天数：</span><p>{{decoration.duration}}天</p></li>
    ...
    <div class="text" v-if="accessoryFiles && accessoryFiles.length > 0"> 
      <div class="description">{{decoration.description}}</div>
      <yd-lightbox :num='accessoryFiles.length'>
        <!-- <yd-lightbox-img v-for="item, key in list" :key="key" :src="item.src"></yd-lightbox-img> -->
        <div class="images">
            <ul>
              <li v-for="(item,index) in accessoryFiles" :key="index"><yd-lightbox-img :src="item.path" alt=""></yd-lightbox-img></li>
            </ul>
        </div>
    </yd-lightbox> 
    </div>
    ...
    <div class="staff" v-if="myrowRecordList.length > 0">
         ...
         <ul>
           <li v-for="(item,index) in myrowRecordList" :key="index">
               <span class="font30"><p class="">检查情况：{{item.description}}</p></span>
               <p class="font28 flex">
    ...
    <yd-timeline class="timeLine" style="padding:0 10px">
        <yd-timeline-item class="timeLineItem" v-for="(item,index) in statusList" :key="index">
    ...
    <div class="flex-row jusCenter center bgwhite border h8 foCoF font32" v-if="tasks && decoration.status == 4" style="margin:.3rem .6rem;border-radius:.1rem;" @click="confirmComplete" :style="{background:mainColor}">装修完成</div>
        ...
        <div class="border">
          ...
          <div style="max-height:5rem;overflow-y:auto">
            <div class="flex-row bt" v-for="(item,index) in record" :key="index" style='padding:0 .2rem;'>
              <span class="nowrap">{{userInfo.name}}</span>
    ...
    <div class="flex-row h9 radius jusCenter font32" v-if="tasks && decoration.status == 3 && userInfo.organType == 1" :style="`background:${mainColor};margin:.3rem .5rem;color:#fff`" @click="rentConfirm" >确定</div>
    
    <!-- 执行人进行装修情况抄单 -->
    <div  @click="xunjian"  class="flex-row h9 bt bgwhite font30" v-if="tasks && decoration.status == 3 && userInfo.organType == 3" style="overflow:hidden" slot="bottom">
   ...
   <yd-layout class="yd-layout" style="width:750px;" v-if="!showInput">
     <div class="title">
        <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{path: '/manage/wuye_service'}">物业服务单</el-breadcrumb-item>
            <el-breadcrumb-item  @click.native='$back'>详情</el-breadcrumb-item>
            <el-breadcrumb-item @click="showInput = true">返回</el-breadcrumb-item>
        </el-breadcrumb>
     </div>
      ...
      <div class="bgwhite record">
        ...
        <div class="flex-row h8">
          <span class="nowrap" style="width:120px">检查日期：</span>
          <el-date-picker
              style="flex:1"
              v-model="recordObj.inspectDate"
              type="datetime"
              :clearable='false'
        ...
        <div class="flex-row mt1">
          <span class="nowrap" style="width:120px">巡查情况：</span>
          <yd-textarea class="border" maxlength='300' style="flex:1;padding:.1rem .2rem .2rem;"  type="text" v-model="recordObj.description">

          </yd-textarea>
        </div>
        <div class="flex-row h8 flex-start">
...
export default {
  ...
  methods: {
    ...
    xunjian() {
      this.record = [];
      this.showAgree2 = true;
    },
    agree1() {
      this.$confirm("是否同意装修").then(res => {
        let data = {
          token: this.$getkey(),
          userId: this.userInfo.guid,
    ...
    csClose() {
      this.$confirm("是否关闭装修").then(res => {
        ...
        this.$ajax(
          `${this.subUrl.activity}/decoration/closeTask`,
          data,
          res => {
    ...
    rentConfirm() {
      ...
      this.$confirm("是否确定完成装修", "提示", { type: "warning" }).then(
        () => {
          this.$ajax(
            `${this.subUrl.activity}/decoration/userConfirm`,
            data,
            res => {
              this.getDecoration();
              this.$message.success('已确定完成')
            },
            this
          );
        }
