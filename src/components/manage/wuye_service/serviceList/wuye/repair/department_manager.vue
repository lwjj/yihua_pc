<template>
<div class=""> 
    <yd-layout class="yd-layout bgwhite">
      <div class="title">
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>选择人员</el-breadcrumb-item>
        </el-breadcrumb>
      </div> 
      <div class="flex-row h8 flex-start pdlr2 bb">请选择人员</div>
      <div class="list-top">
        <ul class="oul" v-for="(it,index) in passW" :key="index" v-show="it.empList.length>0">
          <li @click="showList(index)">
            ...
            <span class="left" style="overflow: hidden">{{it.deptName}}</span>
          </li>
          <ul v-show="it.showList">
            <li class="child-bottom" @click="selectThis(index,no)" v-for="(item,no) in it.empList" :key="no" v-if="it.empList.length>0">
              ...
              <div class="font24 foCo9"  style="padding-left: .3rem;">
    ...
    <!-- <div slot="bottom" class="flex-row bgwhite jusCenter bt h8 foCo6 font32" @click='delPeople'></div> -->
  </yd-layout>
</div>
...
export default {
  ...
  methods: {
    ...
    delPeople() {
      if (!this.isSelect) {
        return this.$message({ message: "请选择员工", type: "error" });
      } else {
