<template>
  <yd-layout class="layout_my">
    <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX" title="租赁" color="#fff" :bgcolor="mainColor"
               height=".88rem">
      <span @click="back" slot="left">
          <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
        </span>
      <div slot="right" @click="$router.push('/estate/serviceList/wuye/lease/add')" class="edit"
    ...
    <yd-infinitescroll :callback="loadList" ref="infinitescrollDemo">
      <div class="contentList" slot="list">
        <div class="list bgwhite pdlr2 mt2" v-for="(item,index) in contentList"  @click="turnTo('/estate/serviceList/wuye/lease/details?id=' + item.guid)" :key="index">
          ...
          <div class="flex-row-nowrap" style="padding:.2rem 0;">
            ...
            <div style="flex:1;">
              <div class="font30"><span class="nowrap">{{item.clientInfo.name.length > 20?item.clientInfo.name.substr(0,20) + '...':item.clientInfo.name}}</span>
              </div>
              <div class="flex font26 foCo9 mt1">
                <span>电话：{{item.clientInfo.account}}</span>

              </div>
            </div>
          </div>
          <div class="flex-row-nowarp h7 bt foCo9 font24">
            <div class="flex foCo9">
              <span>面积需求：</span>
              <span>{{item.area}}平方米</span>
            </div>
            <div class="flex nowrap" style="max-width: 2rem">
              <span>用途：</span>
              <span>{{item.muse.length > 6?item.muse.substr(0,6) + '...':item.muse}}</span>
            </div>
            <div class="flex">
              <yd-icon name='rili' custom size='.4rem'></yd-icon>{{item.createTime.substr(0,10)}}
            </div>
          </div>
        </div>
      </div>
