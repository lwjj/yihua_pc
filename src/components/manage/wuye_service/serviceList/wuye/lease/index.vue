   ...
   <div class="layout_my newRightContent">
    <div class="title">
       <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>租赁</el-breadcrumb-item>
        </el-breadcrumb>
    </div> 
    <div class="tableBox" style="padding-top:10px;">
      <div> 
        ...
        <el-button-group v-if="showBuild" style="float:right;margin-right:10px;">
          ...
          '  @click="$router.push('/estate/serviceList/wuye/lease/add')">新建</el-button>
        </el-button-group>
      ...
      <el-table :data="orderList" height="250" border style="width: 100%">
        ...
        <el-table-column prop="muse" label="用途" :formatter="setRemark" show-overflow-tooltip min-width="14%"> </el-table-column>
        <el-table-column prop="remark" label="备注" :formatter="setRemark" show-overflow-tooltip min-width="13%"> </el-table-column>
        <el-table-column prop="area" label="面积需求" show-overflow-tooltip	min-width="10%" align="center"> </el-table-column>  
        <el-table-column prop="createTime" label="创建时间" show-overflow-tooltip	min-width="17%" align="center"> </el-table-column>   
        <el-table-column label="申请人" min-width="9%" align="center" show-overflow-tooltip	 prop='clientInfo.name' ></el-table-column>
        <el-table-column label="状态" min-width="10%" align="center">
          <template slot-scope="scope">    
            <span class="pdlr1 state state1" v-if="scope.row.status == 1">完成</span>
            <span class="pdlr1 state state2" v-if="scope.row.status == 2">询问客服</span>
            <span class="pdlr1 state state3" v-if="scope.row.status == 3">询问负责人</span>
          </template>
        </el-table-column> 
        <el-table-column label="详情" min-width="8%" align="center"> 
          <template slot-scope="scope">
            <el-button  plain size="small" @click="turnTo('/estate/serviceList/wuye/lease/details?id='+scope.row.guid)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
      ...
      <div id="printcode" v-show="print1">
        <el-table :data="orderList"  border :style="{height:orderList.length*49+48+'px'}">
          <el-table-column prop="guid" label="单号"  width="200px" align="center"></el-table-column>
          <el-table-column prop="muse" label="用途" :formatter="setRemark" align="center" show-overflow-tooltip  width="260px"> </el-table-column>
          <el-table-column prop="remark" label="备注" align="center" :formatter="setRemark" show-overflow-tooltip  width="260px"> </el-table-column>
          <el-table-column prop="area" label="面积需求" show-overflow-tooltip	 width="140px" align="center"> </el-table-column>  
          <el-table-column prop="createTime" label="创建时间" show-overflow-tooltip	 width="200px" align="center"> </el-table-column>   
          <el-table-column label="申请人"  width="160px" align="center" show-overflow-tooltip	 prop='clientInfo.name' ></el-table-column>
          <el-table-column label="状态" width="142px" align="center">
            <template slot-scope="scope">    
              <span class="pdlr1 state state1" v-if="scope.row.status == 1">完成</span>
              <span class="pdlr1 state state2" v-if="scope.row.status == 2">询问客服</span>
              <span class="pdlr1 state state3" v-if="scope.row.status == 3">询问负责人</span>
...
export default {
  ...
  methods: {
    ...
    getQuestionInfo(cb) {
      ...
      if (this.value3) data.type = this.value3;
      console.log(this.value4);
      if (this.value4) data.status = this.value4;
      this.xhr
        .post(
