<template>
  <yd-layout class="layout_my">
    ...
    <div v-if="isShowlist">
      <div class="serviceList"  v-for="(item,index) in serviceList" v-if="item.name == '服务设置' && item.subclass.length > 0">
        <ul @click="serviceSetting(item1.name)" @click="" style="border-right:1px solid #ddd" v-for="item1,index in item.subclass">
          <li>
            <i>
              <img style='width:100%;' :src="item1.icon">
      ...
      <div class="serviceList">
        <ul v-for="(item,index) in serviceList" @click="turnTo(item.name)" :key="index"
            v-if="item.name!='通知' && item.name !='服务设置' ">
          <li>
            <i>
      ...
      if (this.$route.query.token) {
        if (this.userAgent.isAndroid) {
          ...
          setupWebViewJavascriptBridge(function(bridge) {
            ...
            bridge.registerHandler("toIOSApp", function(
              data,
              responseCallback
            ) {
        ...
        .then(res => {
          ...
          if (res.data.code == 1) {
            ...
            if (userType == 3) {
              ...
              console.log('xxx,',this.serviceList)
