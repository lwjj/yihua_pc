<template>
  <yd-layout class="layout_my">
      <yd-navbar class="header" slot="navbar" v-title="'放行条'"  v-show="!userAgent.isWX" title="放行条" color="#fff" :bgcolor="mainColor" height=".88rem">
        ...
        <span @click="addshow" slot="right" class="edit spanRight addRightClickArea" style="color: #fff;font-size:.6rem;">+</span>
      </yd-navbar>
      <div class="addSan" v-show="sanShow" slot="top" @click="toggle2">
       <ul class="font30">
         <li @click="turnTo('/estate/serviceList/release_pass/declares#1')">办理访客出入</li>
         <li @click="turnTo('/estate/serviceList/release_pass/declares#2')">办理物品放行</li>
       </ul>
      </div>
    ...
    <div class="contentList" @scroll="scrollHandler" ref="listDom">
      <div class="content" v-for="(item,index) in contentList" :key="index" @click="turnTo('/estate/serviceList/release_pass/details?guid='+item.guid)">
        <div class="top">
