...
<div class="newRightContent" style="overflow:auto">
 <div class="border bgwhite">
    <yd-layout class="yd-layout" style="width:750px;" v-if="showUser && showTable">
      <div class="title">
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>退租单详情</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      ...
      <div v-if="result.surrender && result.surrender.guid">
        ...
        <div class="staff" v-if="result.meterReadList.length > 0">
          <p class="font28">工程部意见</p>
            <ul>
              <li v-for="(item,index) in result.meterReadList" :key="index">
                <p v-if="item">{{item.project}}</p>
                <p v-if="item">
                ...
                <p  v-if="item">
                  ...
                  <i v-if='item.meterRead'>时间：{{item.meterRead.substr(0,10)}}</i>
                </p>
              </li>
            </ul>
        </div>
        ...
        <div class="aaa">
          <p>
            <span>放行条</span>
            <span v-if="result.surrender">
              <i v-if=" result.surrender.releasePassId">已发放</i>
              <i v-else>待发放</i>
            </span>


          </p>
          <a v-if="result.surrender.releasePassId" :style="{background:mainColor} "  @click="release(1)">查看放行条</a>
          <a v-if="userInfo.organType==1 &&(!result.surrender.releasePassId) && result.surrender.status!=1" :style="{background:mainColor}" @click="release(0)">生成放行条</a>
        </div>
...
export default {
  ...
  methods: {
    release(e) {
      ...
      if (e === 0) {
        this.$router.push(
          `/estate/serviceList/wuye/release_pass/declares?exitId=${
            this.result.surrender.guid
          }`
        );
      } else {
        this.$router.push(
          `/estate/serviceList/wuye/release_pass/details?guid=${
            this.result.surrender.releasePassId
          }`
        );
      }
    },
    confirmComplete() {
      this.$dialog.confirm({
        ...
        opts: () => {
          ...
          this.$ajax(
            `${this.subUrl.activity}/surrender/complete`,
            data,
            res => {
              this.getDetail();
    ...
    zhiConfirm() {
      ...
      this.$ajax1(
        `${this.subUrl.activity}/surrender/meterRead?${this.$qs.stringify(
          data
        )}`,
        this.tableList,
        res => {
    ...
    csConfirm() {
      if (!this.advice.csOpinion) {
        ...
        this.$confirm("该操作会直接将租户进行退租，是否确定要退租", {
          ...
          this.$ajax(
            `${this.subUrl.activity}/surrender/csOpinion`,
            data,
            res => {
              this.$dialog.toast({ mes: "办理退租成功", icon: "success" });
              this.getDetail();
              this.show1 = false;
            },
            this
    ...
    setUserId(e) {
      ...
      if (e) {
        ...
        if (this.lineType == 1) {
          ...
          this.$ajax(
            `${this.subUrl.activity}/surrender/edOpinion`,
            data,
            res => {
    ...
    getDetail() {
      ...
      this.$ajax(
        `${this.subUrl.activity}/surrender/getSurrenderDetail`,
        data,
        res => {
