<template>
  <yd-layout class="layout_my">
      <yd-navbar class="header" slot="navbar" v-title="'装修管理'" v-show="!userAgent.isWX" title="装修管理" color="#fff" :bgcolor="mainColor" height=".88rem">
        ...
        <span @click="turnTo('/estate/serviceList/renovation/declares')" slot="right" class="edit addRightClickArea spanRight" style="color: #fff;font-size:.6rem;">+</span>
      </yd-navbar>
    <!-- <div class="search">
      <div>
        <i @click="searchFn">
    ...
    <div class="contentList" @scroll="scrollHandler" ref="listDom">
      <div class="content" v-for="(item,index) in contentList" :key="index" @click="turnTo('/estate/serviceList/renovation/details?guid='+item.guid)">
        <div class="top">
          ...
          <!-- <span v-show="item.status!=null" :style="{border:'1px solid '+mainColor,color:mainColor}">{{item.statusName}}</span> -->
          <!-- 申请中，2:待审批，3：装修中，4：待执行人确认:5：完成，6：关闭，7：不同意 -->
          <span v-if="item.status == 1" class="pdlr1 state state2">申请中</span>
          <span v-if="item.status == 2" class="pdlr1 state state3">待审批</span>
          <span v-if="item.status == 3" class="pdlr1 state state2">装修中</span>
          ...
          <!-- <span v-if="item.status == 1" class="state state5">申请中</span> -->
        </div>
        <div class="bottom">
...
export default {
  data() {
    return {
      ...
      // 1：申请中，2:待审批，3：装修中，4：完成，5：关闭，6：不同意
      status: [
        { id: 1, name: "申请中" },
        { id: 2, name: "待审批" },
        { id: 3, name: "装修中" },
          ...
          if (res.data.code == 1) {
            if (res.data.data.list != null) {
              res.data.data.list.forEach(element => {
                ...
                if (element.status == 1) {
                  element.statusName = "申请中";
                } else if (element.status == 2) {
                  element.statusName = "待审批";
                } else if (element.status == 3) {
                  element.statusName = "装修中";
