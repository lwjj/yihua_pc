  ...
  <div style="height:100vh">
    <yd-layout class="layout_my" v-if="showUser">
      <div class="blue_box" v-show="!userAgent.isWX">
        <yd-navbar class="header" v-title="'装修申报'" title="装修申报" color="#fff" :bgcolor="mainColor" height=".88rem">
          <span class="spanLeft addClickArea" @click="back" slot="left">
              <span><yd-navbar-back-icon color="#fff"></yd-navbar-back-icon></span>
          </span>
          <span slot="right" @click="submitForApply" class="edit addRightClickArea spanRight" style="color: #fff;font-size:.3rem;">提交</span>
        </yd-navbar>
      </div>
  ...
  methods: {
    ...
    submitForApply() {
      ...
      if (!this.userInfo.unitId) {
        return this.$dialog.toast({ mes: "您暂未入驻单元,请入驻后提交" });
      }
      if (
        this.description == "" ||
        this.datetime1 == "" ||
