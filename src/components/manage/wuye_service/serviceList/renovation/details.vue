...
<div style="height:100vh" >
  <yd-layout class="layout_my" v-if="showUser && showInput">
    ...
    <div class="flex-row h9 radius jusCenter font32" v-if="tasks && decoration.status == 3" :style="`background:${mainColor};margin:.3rem .5rem;color:#fff`" @click="rentConfirm" >装修完成</div>
    <!-- 客服上报 -->
   ...
   <yd-layout class="layout_my yd-layout" style="width:750px;" v-if="!showInput">
      <yd-navbar class="header" slot="navbar" v-title="'装修详情'" v-show="!userAgent.isWX" title="装修详情" color="#fff" :bgcolor="mainColor" height=".88rem">
    ...
    disagree(e) {
      this.$dialog.confirm({
        ...
        opts: res => {
          let url = "";
          if (e == 1) {
            url = `${this.subUrl.activity}/decoration/closeTask`;
            var data = {
              token: this.$getkey(),
              userId: this.userInfo.guid,
              decorationI: this.decoration.guid
            };
          } else if (e == 2) {
