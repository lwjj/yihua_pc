<template>
  <yd-layout class="layout_my">
    <yd-navbar class="header" slot="navbar" :title="title" color="#fff" :bgcolor="mainColor" height=".88rem">
      ...
      <div slot="right" @click="$router.push('/estate/serviceList/settled/add')" class="edit" style="color: #fff;font-size:.6rem;">+</div>
    ...
    <div class="repair-search" v-if="showSearch">
      ...
      <div class="repair-search-div">状态
        <br>
        <div class="div-flex">
          <div v-for="(item,index) in qsStatus" :key="index" @click="selectStatus(index)">
            <div style="width: 100%">
    ...
    <div class="select-tab" v-if="showTab">
      <div @click="select(index)" v-bind:class="selectTab==index?'select-tab-active':''" v-for="(item,index) in tabs" :key="index">{{item}}</div>
    </div>
    ...
    <div class="mClass">
      ...
      <!-- {{item}} -->
      <div class="item-model" v-for="(item, index) in arr" :key="index" v-bind:class="index==0?'top-line':''" @click="showDel(index,item.content)">
        <a @click="toItemDetail(item.guid)">
          ...
          <div class="flex-row" style="padding:.2rem 0;">
            ...
            <div class="flex1" style="flex-flow:column wrap">
              <div class="flex-row pd3 font32" style="width:100%;">
                <div>{{item.clientInfo.organName}}</div>
                <div style="text-align: right" class="state state1" v-if="item.status*1==1">已完成</div>
                <div style="text-align: right" class="state state2" v-if="item.status*1==2">客服受理</div>
              ...
              <div class="flex-row pd3 foCo9" style="width:100%">
    ...
    <div style="text-align: center;color:#999;line-height:.7rem;background:#fff;border:1px solid #ddd;border-left:0;border-right:0" v-if="showErr">
      没有更多数据了!
    </div>
  </yd-layout>
</template>
