...
export default {
  ...
  methods: {
    ...
    buildNew() {
      ...
      // 对表头重新赋值
      this.modelData = JSON.parse(JSON.stringify(this.modelData1));
      //遮罩
      this.showMask = false;
    ...
    getLast() {
      ...
      this.$ajax(
        `${this.subUrl.activity}/manageMonRep/getLastMonthRepDeptDetail`,
        data,
        res => {
          let data = res.data[0];
          if (res.data.length > 0) {
            ...
            var arr = [];
            for (let [i, val] of data.tableRows[0].tableDatas.entries()) {
              arr.push({
                trId: "",
                value: val.value
              });
            ...
            console.log(arr);
            obj.tableRows[0].tableDatas = arr;
            this.modelData = JSON.parse(JSON.stringify(obj));
            this.modelData1 = JSON.parse(JSON.stringify(obj));
          }
        }
      );
    },
