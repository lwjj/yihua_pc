...
<div class="newRightContent" style="overflow:auto">
 <div class="border bgwhite">
    <yd-layout class="yd-layout" style="width:750px" v-show="showReplay && showMuti">
      ...
      <div v-if="result.taskBill" class="foCo6">
        <div class="flex-row-nowarp list_item  bgwhite bb" style="padding: 0.2rem 0.26rem;height:.9rem">
          ...
          <el-button type="primary" v-if="result.tasks && result.taskBill.status == 4 && result.tasks.status == 2" @click="$router.push(`/taskProcess/update/${result.taskBill.guid}`);">更新任务</el-button>
        </div>
        <!--信息-->
      ...
      <div class="h9 flex-row bgwhite jusCenter font32" :style="{color:mainColor,'z-index':2}"
           slot="bottom"
        ...
        <div class="flex1 jusCenter br" v-if="userInfo.postType != 4" style="color:#999" @click="showMuti=false">
        ...
        <div class="flex1 jusCenter" @click="startTask" :style="{color:mainColor}">
          <yd-icon name="selected" size=".46rem" class="mr1 mt1" custom></yd-icon>
          <span>确定</span>
        </div>
      </div>
      <div class="flex-row-nowarp h9 bt bgwhite" slot="bottom" v-if="result.tasks&&result.taskBill.status == 5">
        <div class="flex1 jusCenter" @click="endTask" :style="{color:mainColor}">
          <yd-icon name="selected" size=".46rem" class="mr1 mt1" custom></yd-icon>
          <span>验收任务</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    disagree() {
      this.$confirm("是否驳回任务", "提示", { type: "warning" }).then(() => {
        let data = {
          token: this.$getkey(),
          userId: this.userInfo.guid,
          taskBillId: this.result.taskBill.guid,
    ...
    startTask() {
      this.$confirm("是否启动任务", "提示", { type: "warning" }).then(() => {
        let data = {
          token: this.$getkey(),
          userId: this.userInfo.guid,
          taskBillId: this.result.taskBill.guid,
    ...
    endTask() {
      this.$confirm("是否验收任务", "提示", { type: "warning" }).then(() => {
        let data = {
          token: this.$getkey(),
          userId: this.userInfo.guid,
    ...
    confirm(e) {
      ...
      if (e) {
        ...
        this.$ajax(
          ...
          }/workbench/taskBill/myReport/eplyReportDetail`,
          data,
          res => {
