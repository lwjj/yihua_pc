...
<div class="newRightContent">
  <div class="border bgwhite">
    <div>
    <yd-layout class="yd-layout" v-show="showUser && showTable &&showMuti" style="width:750px;">
      ...
      <div class="usuallyBox bb" style="padding: .1rem 0 .1rem;">
        <div class="list_item">
          ...
          <!-- <input type="number" oninput="value = this.value.slice(0,3)" onkeyup="value = this.value.slice(0,3)" maxlength="3"> -->
          <el-input type="text" :maxlength="3" style="border:0;flex:1" placeholder="修改百分比" v-model="progress"></el-input>
        </div>
        <div class="list_item mt2" style="height: auto;align-items: flex-start;padding-bottom: .2rem">
          <div class="label">备注</div>
          <el-input type="textarea"  :maxlength="300" style="border:0"
                       class="border foCo9 radius"
                       placeholder="非必填项" v-model="remark">
      ...
      <div class="flex-row-nowarp h9 bgwhite bt bd mt2 " slot="bottom">
        <div class="flex1 jusCenter br" @click="confirm(1)">更新进度</div>
        <div class="flex1 jusCenter" @click="confirm(2)" :style="{color:mainColor}">
          <yd-icon name="selected" custom size=".4rem"></yd-icon>
          <span>完成任务</span>
        </div>
      </div>
    </yd-layout>
      ...
      if (e == 1) {
        ...
        if (this.progress == "") {
          ...
          this.$ajax(
            ...
            data,
            res => {
              this.$dialog.toast({ mes: "已确认完成任务", icon: "success" });
              this.$router.go(-1);
            },
            this
          );
