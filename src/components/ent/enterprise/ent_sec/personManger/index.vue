...
<div class="newRightContent" style="overflow:auto">
 ...
 <yd-layout class="yd-layout">
    <!-- <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX" title="部门管理" color="#fff"
               :bgcolor="mainColor"
               height=".88rem">
    ...
    <div class="title">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item @click.native="$router.go(-1)">工作台</el-breadcrumb-item>
      ...
      <ul class="oul" v-for="(it,index) in passW" :key="index">
        ...
        <ul v-show="it.showList">
          <li class="child-bottom" @click="selectThis(index,no)" v-for="(item,no) in it.empList" :key="no"
            ...
            <img class="list-item" src="../../../../../assets/images/rent/ent/zhijiao.png"
                 alt="">
            <div>
              ...
              <div class="flex font24 foCo9">
                <span class="l-center font24" v-if="item.post">岗位：{{item.post}}</span>
                <span  class="l-center font24" v-if="!item.post">岗位：暂无</span>
              </div>
            </div>
  ...
  methods: {
    ...
    delPeople() {
      if (!this.isSelect) {
        ...
        this.$emit("setAssignId", this.myUser);
      }
    },
    selectThis(index, no) {
      var flag = true;
    ...
    sureDelDept(guid) {
      this.xhr
        .post(
          ...
          this.organId
            ? this.organId
            : this.organInfo.guid + "&DepartmentId=" + guid
        )
        .then(res => {
          if (res.data.code == 1) {
    ...
    getDepartmentInfo(cb) {
      let data = {
        token: this.$getkey(),
        organId: this.organId ? this.organId : this.organInfo.guid
      };
      this.$ajax(
      ...
      // this.xhr.post(`${this.subUrl.user}/admin/deptManage/organDepartmentInfo?token=` +  + '&organId=' + this.organId?this.organId:this.organInfo.guid).then((res) => {
      //   if (res.data.code == 1) {
      //     console.log(res)
