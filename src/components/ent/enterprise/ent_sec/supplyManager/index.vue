   ...
   <div class="layout_my newRightContent">
    ...
    <div class="tableBox">
      <div class="tableTop"> 
        ...
        <el-button-group v-if="showSend">
          <el-button type="primary" icon='el-icon-plus
          ' @click="turnTo('/supplyManager/add')">新建</el-button> 
          ...
          width='180' 
           ...
           <!-- //状态 0:关闭, 1:完成, 2:草稿 3:待批 4:同意 5:不同意 -->
            <template slot-scope="scope"> 
              ...
              <span v-if="scope.row.status == 3" class="pdlr1 state state2">待批</span>
              <span v-if="scope.row.status == 4" class="pdlr1 state state1">同意</span>
              <span v-if="scope.row.status == 5" class="pdlr1 state stateClose">不同意</span>
           </template>
        </el-table-column> 
        <el-table-column
          label="详情"
          ...
          min-width="180"
           > 
           <template slot-scope="scope">
             <el-button type="primary" size="mini" @click="turnTo('/supplyManager/detail?guid='+scope.row.guid)">详情</el-button>
             <el-button type="danger" size="mini" v-if="scope.row.status == 3" @click='deleteItem(scope)'>删除</el-button>
           </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    deleteItem({ row, index }) {
      this.$confirm("是否删除当前采购管理", "提示", { type: "warning" })
        .then(() => {
          let data = {
            token: this.$getkey(),
            pasaId: row.guid
    ...
    getQuestionInfo(mType) {
      ...
      this.xhr
        .post(`${this.subUrl.activity}/pasa/pasaList`, this.$qs.stringify(data))
        .then(res => {
          if (res.data.code == 1) {
    ...
    getSearch() {
      ...
      this.xhr
        .post(
          `${this.subUrl.activity}/pasa/pasaList?` + qs.stringify(this.data)
        )
        .then(res => {
          if (res.data.code == 1) {
  ...
  mounted() {
    ...
    this.$getRoot(res => {
      for (var i of res.entService.subclass) {
        if (i.name == "工作台") {
          for (var j of i.subclass) {
            if (j.name == "采购管理") {
              if (j.subclass.length > 0) {
                this.showSend = true;
              }
            }
          }
