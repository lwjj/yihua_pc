   ...
   <div class="layout_my newRightContent">
    <div class="title">
       <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>工作汇报</el-breadcrumb-item>
        </el-breadcrumb>
    </div> 
    <div class="tableBox">
      <div class="tableTop"> 
        ...
        <!-- <el-button-group>
          ...
          ' @click="turnTo('/supplyManager/add')" v-if="this.userInfo.postType >= 3">新建</el-button> 
        </el-button-group> -->
         <el-dropdown  v-if="userInfo.postType*1 >= 3">
            <el-button type="primary">
              新建<i class="el-icon-plus el-icon--right"></i>
            </el-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item @click.native="$router.push('/taskreplay/add?type=1')">新建日报</el-dropdown-item>
              <el-dropdown-item @click.native="$router.push('/taskreplay/add?type=2')">新建周报</el-dropdown-item>
              <el-dropdown-item @click.native="$router.push('/taskreplay/add?type=3')">新建月报</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
      </div> 
      <el-table
          ...
          width='180' 
           ...
           <!--  0：未阅 1：已阅 2：草稿 , -->
            <template slot-scope="scope"> 
              <span v-if="scope.row.status == 0" class="pdlr1 state state2">未阅</span>
              <span v-if="scope.row.status == 1" class="pdlr1 state state2">已阅</span>
        ...
        <el-table-column
          label="详情"
           > 
           <template slot-scope="scope">
             <el-button type="primary"  @click="turnTo('/taskreplay/detail/' + scope.row.guid + '?type=' + scope.row.type,scope.row)"
            >详情</el-button>
           </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    turnTo(url, e) {
      ...
      if (e.status == 2) {
        this.$router.push(
          `/taskreplay/add?type=${e.type}&handle=2&wrId=${e.guid}`
        );
      } else {
        //跳转到详情
        if (this.userInfo.postType == 2) {
          ...
          if (
            this.userInfo.postType == 2 ||
            (this.userInfo.postType == 3 && this.buttomType == 0)
          ) {
            url = url + `&isApply=1`;
          }
        }
