...
<div id='subTemplate' v-if="loading">
    ...
    <div class="del" v-show="show">
      <ul class="delList clearfix">
        ...
        <li v-if="taskDel.taskBill.startDate">起止时间:<span v-text="taskDel.taskBill.startDate.split(' ')[0]"></span></li>  
        <li v-if="taskDel.taskBill.createDate">创建时间: <span v-text="taskDel.taskBill.createDate.split(' ')[0]"></span></li>
        <li>类型:<span>{{taskDel.taskBill.typeName}}</span></li>
        <li>单号:<span>{{taskDel.taskBill.guid}}</span></li>
      </ul>
      <div class="info">
        <p>{{taskDel.taskBill.taskDetail}}</p>
        <vue-preview class='imgPath' :slides="sliders" v-if='sliders'  @close="''"></vue-preview></div>
      <div class="people">
      ...
      <div class="submitBtn" v-if="submitShow">
        ...
        <el-button type="primary" @click="assign()">指派人</el-button>
