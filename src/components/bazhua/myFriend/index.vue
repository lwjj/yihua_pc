  ...
  <div id="new_progress">
    <div class="title">
      ...
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>圈子好友</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-col class="content" v-if="showInfo">
      ...
      <div class="topTitle">
        ...
        <div class="flex">
          <el-input v-model="input" class="mr1" placeholder="请输入查询人"></el-input>
          <el-button plain @click.native="getList">查询</el-button>
        </div>
      </div>

        <div class="mt2"></div>
        <el-col  v-for="(item,index) in tipList" :key="index" class="tip-detail" :span="24">
          <div class="myTitle">
            <img style="border-radius:50%" @click="toUserDetail(item.userId2)" :src="item.headPortrait" width="48" height="48" :alt="item.nickname">
            <div class="mRight">
              <div class="m-top">
                <div style="line-height:1.8">{{item.nickname}}</div>
                <div style="color:#999" >备注：{{item.remarck||item.remarck==''?item.remarck:'无'}}</div>
                <div>描述：{{item.description||item.description==''?item.description:'暂无好友描述！'}}</div>
                <div>联系方式：{{item.account?item.account:'暂无联系方式'}}</div>
              </div>
              <!-- you右边添加好友 -->
              <div class="iconGroup">
                <!-- <img @click.stop="open1(1,item.replyUserId)" class="mr1 cursor" title="添加好友"  width="25"
                  src="../../../assets/images/circle/add.jpg" alt=""> -->
              <!-- <img @click.stop="open1(2,item.userId1,item.userId2)" class="cursor" title="拉黑好友" width="25"
                  src="../../../assets/images/circle/black.jpg" alt=""> -->
                   <img v-if="item.status==1" width="25" class="cursor" title="拉黑好友" @click="open1(2,item.userId1,item.userId2)"
               src="../../../assets/images/circle/addblack.png" alt="">
              <img v-if="item.status==2" width="25" class="cursor"  title="拉白好友" @click="open1(1,item.userId1,item.userId2)"
                  src="../../../assets/images/circle/remove_black.png" alt="">
              </div>
            </div>
          </div>
          <div class="myContent" v-if="item.bbs">
            {{item.bbs.content.length > 200?item.bbs.content.substr(0,200)+'...':item.bbs.content}}
          </div> 
        </el-col>
        </el-col>
         <el-col class="bgwhite noData pb2" v-if="!tipList || tipList.length==0">
            暂无数据！
        </el-col>
      <el-col class="bgwhite page">
        <el-pagination
          v-if="bm.rowCount > 0"
          background
          style="float:right;"
    ...
    <!-- </el-col> -->
    <el-col v-if="!showInfo" class="content">
        <el-col>
          <el-col class="tip-detail"
                  style="">
            <el-col :span="2" style="min-width: 100px;cursor: pointer"><img @click="showUserInfo(friendInfo.user.userId2)"
                                                                            style="width: 100px;border-radius: 50%" :src="friendInfo.user.headPortrait"
                                                                            alt=""></el-col>
            ...
            <el-col :span="6" style="margin-left: 1rem;margin-top: 1.5rem;cursor: pointer">
