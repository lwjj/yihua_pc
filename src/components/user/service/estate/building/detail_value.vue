<template>
  <div class="" >
      <div>
        <div class="title">
          <el-button  @click.native="mClose" >返回</el-button>
        </div>
        ...
        <div class="mt2 bt bb bgwhite center flex-start pdlr2">
            ...
            <div class="flex" style="width:400px">
              <div style="width:50%;margin:.2rem" v-for="(item,index) in couponList" :style="activeIndex==index?`border-color:${mainColor};color:${mainColor}`:``" :key="index" class="flex cursor h7 chooseCoupon jusCenter border" @click="chooseCoupon(index)">{{item.name}}</div>
            </div>
            <div class="flex h8 flex-start">
                <img class="chooseIcon" src="../../../../../assets/images/rent/estate/money.png" alt="">
                <div class="fuColor3">{{cost*10}}商机点</div>
                <span></span>
            </div>
            <yd-cell-group style="width:500px">
          ...
          <div class="h7 pdlr2 left">有效期至：{{deadline}}</div>
        </div> 
        <div class="flex h9 jusCenter radius" @click="confirm" style="margin:.8rem .4rem;width:200px;" :style="{color:'#fff',background:mainColor}">确定开通</div> 
    </div>
  </div>
</template>
<script>
