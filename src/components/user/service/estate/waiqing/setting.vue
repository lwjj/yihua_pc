      ...
      <div class="list">
				<yd-cell-group v-for="(item,index) in list" :style="index<=3?'margin-bottom:.2rem':'margin:0'" :key="index">
					...
					<div class="flex-row-nowrap h7 bgwhite" v-if="item.startTime" style="padding:.2rem .5rem;box-sizing:content-box;">
						<el-time-select v-model="item.startTime" @change="changeData($event,index)" class="mr2" :picker-options="optionsDate"  placeholder="选择时间"></el-time-select>
						<el-time-select v-model="item.endTime" @change="changeData($event,index)" :picker-options="optionsDate"  placeholder="选择时间"></el-time-select>
