   ...
   <div class="newRightContent">
      <div class="title">
       <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item  @click.native="$router.go(-2)">返回</el-breadcrumb-item>
            <el-breadcrumb-item>设置行业类型</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
     <!-- <yd-navbar slot="navbar"  height=".88rem"  v-title="'设置行业分类'" title="设置行业分类" :bgcolor="mainColor" color="#fff">
     ...
     <div class="content">
         ...
         <div class="list">
           ...
           <div class="subContent" v-show="item.show">
             <yd-button class="buttonChoose" v-for="(item1,key1) in item.subclass" :key="key1"
                        :type="item1.show?'primary':'hollow'"   :bgcolor='item1.show?mainColor:""' :color='item1.show?"#fff":""' @click.native="choose_free(key,key1)">{{item1.name}}
             </yd-button>
           </div>
         </div>
       </div>
       ...
       <div class="bgwhite pdlr2 pb2">
         <el-button class="mt2" type="primary" @click="chooseSuccess()">确定</el-button>
       </div>
     </div>
    ...
    chooseSuccess() {
      ...
      if (this.requireObj) {
        ...
        if (
          ...
          let data = {
            token: this.$getkey(),
            proId: obj.require.guid
          };
          this.$ajax(
            `${this.subUrl.activity}/serviceOrder/addPro`,
            data,
            res => {
              this.$message({
                message: "设置成功",
