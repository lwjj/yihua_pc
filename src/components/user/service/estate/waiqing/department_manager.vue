...
<div style="overflow:auto">
    <yd-layout class="yd-layout" v-show="showAdd"> 
      <div class="list-top">
        <ul class="oul" v-for="(it,index) in passW" :key="index">
          <ul>
            <li class="child-bottom" @click="selectThis(index,no)" v-for="(item,no) in it.empList" :key="no"
              ...
              <!--<div  slot="left" @click.stop="edit(item)" style="padding: .2rem;">-->
    ...
    <el-button class="ml2 mt2 mb2" @click='confirm' type="primary">确定</el-button>
...
export default {
  ...
  methods: {
    ...
    confirm() {
      ...
      this.$emit("setUserId", this.chooseUser);
    },
    sendToOne() {
      // alert(1);
      var data = {
    ...
    delPeople() {
      ...
      if (flag) {
        ...
        this.$emit("sendCode", this.toDel);
      } else {
        return this.$dialog.toast({ mes: "请选择用户", timeout: "600" });
      }
    },
    selectAll() {
      ...
      for (var i = 0; i < this.passW.length; i++) {
        for (var j = 0; j < this.passW[i].empList.length; j++) { 
          if (i == index && no == j) {
            ...
            this.chooseUser = this.passW[i].empList[j];
          } else {
    ...
    getDepartmentInfo(cb) {
      ...
      this.xhr
        .post(
          `${
            this.subUrl.user
          }/admin/merchant/getMemberlist?${this.$qs.stringify(data)}`
        )
        .then(res => {
          if (res.data.code == 1) {
            console.log(res);
