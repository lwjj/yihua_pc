...
export default{
    ...
    methods:{
        getMain(pageNo){
            ...
            this.$ajax(`${this.subUrl.user}/asset/getBuyLog`,
            data,
            res=>{  
                ...
                for(var p=0;p<this.tableData.length;p++){
                    if((this.tableData[p].puyType==1)||(this.tableData[p].puyType==5)||(this.tableData[p].puyType==6)){
                        this.tableData[p].puyType="微信支付"
                    }else if((this.tableData[p].puyType==2)||(this.tableData[p].puyType==7)){
                    ...
                    if(this.tableData[p].businessPoint!==null){
                        this.tableData[p].businessPoint=this.tableData[p].businessPoint+"个商机点"
                    }else{
                        this.tableData[p].businessPoint="0个商机点"    
                    }

                    this.tableData[p].totalFee=this.tableData[p].totalFee.toFixed(2)+"元"

                    this.tableData[p].status=this.tableData[p].status==0?"失败":(this.tableData[p].status==1?"成功":"未支付")        
                }
