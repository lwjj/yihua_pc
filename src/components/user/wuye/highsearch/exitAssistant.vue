  ...
  <div class="newRightContent">
      <div class="title">
        <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item>退出助手</el-breadcrumb-item>
        </el-breadcrumb>
    </div>
      <div class="bgwhite border">
          <div style="padding:20px">
              <el-button type="primary" @click="exitSearch">退出助手</el-button>
        </div> 
      </div>
  </div>
</template>
...
export default {
  data() {
    this.tempObj = JSON.parse(
      decodeURIComponent(localStorage.getItem("tempObj1"))
    );
    return {};
  },
  methods: {
    exitSearch() {
      this.$confirm("是否退出助手", "提示", { type: "warning" }).then(
        () => {
          ...
          localStorage.removeItem('tempObj1')
          this.$router.push("/manage/wuye_service");
