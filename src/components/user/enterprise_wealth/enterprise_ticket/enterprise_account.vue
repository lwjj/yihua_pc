        ...
        <div class="box">
            ...
            <div class="info">
                ...
                <el-row style="margin-top:10px;border-bottom:1px solid #ddd; padding-bottom:10px;" v-for="item in this.dataTable" :key="item.guid">
                    <el-col :span="3" style="font-size:14px; font-weight:bold">{{item.type}}</el-col>
                    <el-col :span="13" style="margin-top:2px;margin-left:7%;">微信账号：{{item.account}}</el-col>
                    <el-col :span="3" style="text-align:right;margin-right:10px;"><el-button size="mini" type="primary" @click="deltSan(item.guid)">解绑账号</el-button></el-col>
                    <el-col :span="3"><el-button size="mini" type="primary" @click="defaultSan(item.guid)">{{item.status}}</el-button></el-col>
                </el-row>
            </div>
        </div>
    </div>
...
export default {
  ...
  methods: {
    getCollectionName() {
      ...
      this.$ajax(
        `${this.subUrl.user}/asset/getSanList`,
        data,
        res => {
          ...
          for (var i = 0; i < this.dataTable.length; i++) {
            this.dataTable[i].type =
              this.dataTable[i].type == 1 ? "微信支付" : "银行卡支付";
            this.dataTable[i].status =
              this.dataTable[i].status == 1 ? "默认账号" : "设为默认";
          }
    ...
    defaultSan(sanId) {
      this.$confirm("此操作将转换默认收款账户, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$ajax(
            `${this.subUrl.user}/asset/setDefaultSan`,
            data,
            res => {
              if (res.code == 1) {
