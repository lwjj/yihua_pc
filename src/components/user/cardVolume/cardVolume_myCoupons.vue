  ...
  <div class="couponBox">
      <div class="tableContent">
        <el-table height="100%" :data='tableData' border>
          ...
          <el-table-column label="使用期限" prop="slogan" width="200" show-overflow-tooltip>
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" fixed="right" min-width="180">
            <template slot-scope="scope">
              <el-button-group>
                <el-button type="primary" @click="goView(scope.row.expandTitle,scope.row.guid,scope.row.ruleIntroduction,scope.row.startDate,scope.row.endDate,scope.row.usableTime)" size='mini'>查看二维码</el-button>
...
export default {
  ...
  methods: {
    ...
    deleteItem(e) {
      this.$confirm("是否删除当前卡券", { type: "warning" }).then(() => {
        var data = {
          token: this.$getkey(),
