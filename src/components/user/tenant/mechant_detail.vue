  ...
  <div id="new_progress">
   <div v-if="!showMechant">
    ...
    <el-col v-if="details.isLookPhone*1>0">
      <div class="declares" v-show="details.isRegister==0&&details.userId!=userInfo.guid">
        <!-- <ol>
        ...
        <div >
          ...
          <!-- <div><p>我的报价</p><span class="bb"><input v-model="myOffer" type="number" oninput="value= this.value.slice(0,9)" onkeyup="value=this.value.slice(0,9)"></span></div>
          <div>
            <p>我的附言</p>
            <i class="">
              <yd-textarea slot="right" v-model="myExplain" placeholder="说明/附言" maxlength="300"></yd-textarea>
            </i>
          </div> -->
        </div>
      </div> 
    ...
    <!-- 报价及推荐按钮隐藏 -->
          <div class="list_item mt1" style="margin-top:0">
            <!-- 报价按钮 -->
              <div class="label"></div>
              <div v-show="details.isRegister==0&&details.userId!=userInfo.guid">
                <el-button type="primary"  @click="addRegister" style="margin-right:120px" v-show="details.isLookPhone==1">报价</el-button>
              </div>
            <!-- 推荐按钮 -->
            <div style="padding:20px 0px;background:#fff">
                <el-button type="primary" @click="showList" v-if="!show">推荐商家</el-button>
            </div>
          </div>
    ...
    <mechantList v-if="showMechant" @checkMen='checkMen'></mechantList>
  </div>
</template>
<script>
import Vue from "vue";
import mechantList from "./mechantList";
    ...
    checkMen(e) {
      ...
      if (e && e.length > 0) {
        ...
        var data = {
          ...
          type: this.$route.query.type == 2 ? 6 : 7
        };
        this.$ajax(`${this.subUrl.bd}/buildingBuy/recommendBusiness`,data,res=>{this.$message.success('推荐成功')},this)
      }
      this.showMechant = false;
    },
    openConfrim() {
      this.$confirm("是否支付商机点查看联系方式", "提示", {
