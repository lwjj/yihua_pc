  ...
  <div id="new_progress">
    ...
    <div style="margin-top: 10px">
      ...
      <yd-layout v-show="showshow" class="edit">
        <!--<yd-navbar height=".88rem" slot='navbar' v-show="!userAgent.isWX" v-title="'推送信息'" title="推送信息"-->
        ...
        <div class="declares" style="margin:.2rem 0">
          ...
          <div
            style="height:.8rem;display:flex;justify-content: space-around;border-bottom:1px solid #F3F3F2;align-items:center;">
          ...
          <div class="textar">
            <div><p style="width:90px;display:inline-block;">标题</p><span class="bb"><input maxlength="35" v-model="pm.expandTitle" placeholder="推广" type="text"></span></div>
            <div>
              <p style="width:90px;display:inline-block;">规则介绍</p>
              <el-input  style="width:624px;" v-model="pm.ruleIntroduction" placeholder="文本内容" maxlength="100"  type="textarea" :autosize="{ minRows: 2, maxRows: 4}"></el-input>
            </div>
          </div>
        </div>
          ...
          <div class="bottoms">
            <ul v-for="(item,index) in typeList" :key="index">
              ...
              <li>
                ...
                <p v-if="index==1&&pm.organIdList==''" class="textHidden" style="color:#999;overflow: hidden" @click="toggleH">点击选择客户</p>
                <p v-if="index==1&&pm.organIdList!=''" class="textHidden" style="color:#999;overflow: hidden" @click="toggleH">
                  {{pm.organIdList}}</p>
                <p v-if="index==2">
        ...
        <div class="to-select">
          <div class="buttonBox">
            <yd-button size="large" class="send" @click.native="addPushInfo" :bgcolor="mainColor" color='#fff'
                       type="primary">
              立即推送
            </yd-button>
          </div>
  ...
  export default {
    ...
    handleRemove(file, fileList) {
      ...
      addPushInfo() {
        ...
        this.$ajax1(
          `${this.subUrl.bd}/opportunity/addPushInfo?token=` + this.$getkey() + '&type=' + this.clickId +
