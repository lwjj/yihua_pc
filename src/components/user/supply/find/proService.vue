    ...
    <div class="main">
      ...
      <el-col v-if="details.organInfoDto3"
        ...
        <el-col v-if="details.isGroup!=0">
          <el-col
            style="line-height: 35px;color: #f2725e;border-bottom: 1px solid #f3f6f5;border-top: 1px solid #f3f6f5;" v-if="details.paGroupStatus == 2">
            团购未开始
          </el-col>
          <el-col
            style="line-height: 35px;color: #f2725e;border-bottom: 1px solid #f3f6f5;border-top: 1px solid #f3f6f5;"  v-if="details.paGroupStatus == 1">
            团购进行中
          </el-col>
          <el-col
            style="line-height: 35px;color: #f2725e;border-bottom: 1px solid #f3f6f5;border-top: 1px solid #f3f6f5;"  v-if="details.paGroupStatus == 0">
            团购已结束
          </el-col>
        ...
        <el-col v-if="userInfo.guid!=details.userId" style="display: flex;margin: 0.3rem;width: 300px">
          ...
          <el-button v-if="details.paGroupStatus == 1" size='small' type='primary' @click.native="toShowBg(1)">参加团购</el-button>
        </el-col>
      </el-col>
    </div>
    <div v-if="showAlert" class="productAlert">
      <div>
        <div style="color: #999999;line-height: 30px;" v-if="toJoin&&type!=3">
          <div class="alertTitle">{{type == 2?'预约':'参团'}}</div>
          <div style="display: flex;margin: 0.1rem;">
            <div class="label">姓名：&nbsp;</div>
            <input style="flex: 4;line-height: 30px" placeholder="请填写" type="text" v-model="data.name">
          </div>
          <div style="display: flex;margin: 0.1rem;">
            <div class="label">电话：&nbsp;</div>
            <input v-on:input="isPhone" style="flex: 4;line-height: 30px;width: 80%;" placeholder="请填写" type="text" v-model="data.phone">
            <el-button @click='getCode' sytle='padding:9px 20px;margin-left:10px'  v-if='flag=="true"&&sendAuthCode'>获取验证码</el-button>
            <el-button sytle='padding:9px 20px;margin-left:10px'  v-if='flag=="true"&&!sendAuthCode'>{{count}}秒后重新获取</el-button>
            
          </div>
           <div v-if='flag=="true"' style="display: flex;margin: 0.1rem;">
            <div class="label">验证码：&nbsp;</div>
            <input v-model="data.code" style="flex: 4;line-height: 30px" placeholder="请填写" type="text">
          </div>
          <div style="display: flex;margin: 0.1rem;">
            <div class="label">地址：&nbsp;</div>
            <input style="flex: 4;line-height: 30px" placeholder="请填写" type="text"  v-model="data.address">
          </div>
          <div style="display: flex;margin: 0.1rem;">
            <div class="label">备注：&nbsp;</div>
            <textarea style="flex: 4;line-height: 30px;border-color:#ddd;" placeholder="请填写" type="text"  v-model="data.remarks"></textarea>
          </div>
          <div style="display: flex;margin: 0.1rem;">
            <div class="label"></div>
...
//  	console.log(this.data.phone)
    	if(this.data.phone!=this.userInfo.account){
    		this.flag='true'
    	}else{
    		this.flag='false'
    	}
    ...
    getCode(){
      ...
      let data={
      	sign:'e0d3bffd4af3cbd79e04383a664a71ff',
      	AppToken:'YiHua/1.0/mobile/uuid/1540001803880',
    ...
    hidBg() {
    	...
    	this.data.type='';
    	this.data.code='';
      this.data.phone=this.userInfo.account;
      this.flag='false';
      this.showAlert = false;
      this.type = 0;
    },
